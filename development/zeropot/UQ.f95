...
double precision :: gap , ketot , sumsq=0. , nucradi , zeropot
...
read(24,*) elec , atomnum , atomkount , x , y , z , ntrials , nbin , expanse
...
!.. calculates zero potential based on size of super cell
zeropot = (dfloat(1)/sqrt(((dfloat(expanse+1)*x)**2)+((dfloat(expanse+1)*y)**2)+((dfloat(expanse+1)*z)**2)))
write(*,*) zeropot
...
if (f >= g) then 
peri = periodic(pos(f,1),pos(f,2),pos(f,3),pos(g,1),pos(g,2),pos(g,3),x,y,z,dfloat(atomnum**2),expanse,zeropot)
...
nuc_nuc_inter = nuc_nuc_inter + ((dfloat(atomnum**2)/gap) - (dfloat(atomnum**2)*zeropot))
...
electrons(q,2),electrons(q,3),x,y,z,dfloat(1),expanse,zeropot)
...
elec_elec_inter = elec_elec_inter + ((dfloat(1)/gap) - (dfloat(1)*zeropot))
...
peri = periodic(electrons(u,1),electrons(u,2),electrons(u,3),pos(v,1),pos(v,2),pos(v,3),x,y,z,dfloat(atomnum),expanse,zeropot)
		
elec_nuc_inter = elec_nuc_inter + peri
		
gap = twopoints(electrons(u,1),electrons(u,2),electrons(u,3),pos(v,1),pos(v,2),pos(v,3))
		
elec_nuc_inter = elec_nuc_inter + ((dfloat(atomnum)/gap) - (dfloat(atomnum)*zeropot))
